...
export const Renderer = (() => {
  ...
  let flashAlpha=0, flashColor=CFG.FG, gridPulse=0;
  let levelText='', levelAlpha=0, edgeDanger=0;
  let crumbleActive=false, crumbleSegs=[];

  // ── Interpolação suave ─────────────────────────────────────────
  // Armazena posições interpoladas de cada segmento
  const _interp = {
    positions: [],    // [{px,py}] — posições de pixel interpoladas
    prevSnap:  [],    // snapshot anterior (grid coords)
    currSnap:  [],    // snapshot atual (grid coords)
    lastTickTime: 0,  // quando ocorreu o último tick do engine
    tickInterval: 145,// espelha _currentSpeed (atualizado via Bus)
    alpha: 0,         // 0..1 progresso de interpolação
  };
  ...
  // ── Paredes dinâmicas — cache de animação (fade in/out) ────────
  const _dynWallAnim = new Map(); // key=`${x},${y}` → {alpha, state:'in'|'out'}
  const DYN_FADE = 600; // ms do fade

  function _dynWallIn(cells) {
    cells.forEach(c => {
      const k=`${c.x},${c.y}`;
      _dynWallAnim.set(k,{alpha:0, phase:'in', born: performance.now()});
    });
    // Partícula de spawn
    if (canvas) {
      const cx=(cells[0].x+cells[cells.length-1].x)/2*CFG.CELL+CFG.CELL/2;
      const cy=(cells[0].y+cells[cells.length-1].y)/2*CFG.CELL+CFG.CELL/2;
      particles.ring(cx,cy,{count:10,speed:2,radius:4,color:'#ff6b35',glow:6,decay:[.04,.08]});
    }
  }
  function _dynWallOut(cells) {
    cells.forEach(c => {
      const k=`${c.x},${c.y}`;
      const existing=_dynWallAnim.get(k);
      _dynWallAnim.set(k,{alpha: existing?existing.alpha:1, phase:'out', born: performance.now()});
    });
  }

  Bus.on('dynWallSpawned', ({cells}) => _dynWallIn(cells));
  Bus.on('dynWallExpired', ({cells}) => _dynWallOut(cells));
  Bus.on('dynWallsReset',  ()        => _dynWallAnim.clear());
  ...
  function init(targetCanvas, stateRef, engineRef) {
    ...
    _running=true; _makeGrain();
    _loop();
    console.log('[Renderer] %dx%d px',canvas.width,canvas.height);
  }
  ...
  function onEat(food, combo) {
    ...
    if(food.type!=='cherry'&&food.type!=='apple'){shake.trigger(4,10);particles.ring(wx,wy,{count:12,speed:2.2,color:food.color||CFG.FG});}
    const head=gameState?.snake?.[0];if(head)_snakeTrail.unshift({x:head.x,y:head.y,alpha:.3});
    // Captura snapshot para interpolação
    _snapSnake();
  }
  ...
  function clearEffects(){particles.clear();shake.reset();animator.cancelAll();flashAlpha=gridPulse=levelAlpha=edgeDanger=0;_snakeTrail.length=0;crumbleActive=false;crumbleSegs=[];_interp.positions=[];_interp.prevSnap=[];_interp.currSnap=[];_dynWallAnim.clear();}
  ...
  // ── Interpolação suave ─────────────────────────────────────────
  // Chamado pelo Bus 'stateUpdate' para capturar o snapshot atual
  function _snapSnake() {
    if (!gameState?.snake) return;
    _interp.prevSnap = [..._interp.currSnap];
    _interp.currSnap = gameState.snake.map(s=>({x:s.x,y:s.y}));
    _interp.lastTickTime = performance.now();
    // Atualiza tickInterval com a velocidade real do engine se disponível
    if (gameEngine?._internals?._currentSpeed !== undefined) {
      _interp.tickInterval = gameEngine._internals._currentSpeed;
    }
  }

  Bus.on('stateUpdate', () => {
    // Só actualiza snapshot se a cobra cresceu ou moveu (não quando está pausada)
    if (gameState?.running && !gameState?.paused) _snapSnake();
  });

  function _computeInterp(now) {
    if (!_interp.prevSnap.length || !_interp.currSnap.length) {
      _interp.positions = (_interp.currSnap||[]).map(s=>({px:s.x*CFG.CELL, py:s.y*CFG.CELL}));
      return;
    }
    const elapsed = now - _interp.lastTickTime;
    // alpha clamped 0..1 com easeOut leve
    let t = Math.min(elapsed / Math.max(_interp.tickInterval, 50), 1);
    t = Ease.easeOut(t); // suaviza a aceleração

    const maxLen = Math.max(_interp.prevSnap.length, _interp.currSnap.length);
    _interp.positions = [];

    for (let i=0; i<maxLen; i++) {
      const cur  = _interp.currSnap[i]  || _interp.currSnap[_interp.currSnap.length-1];
      const prev = _interp.prevSnap[i]  || _interp.prevSnap[_interp.prevSnap.length-1] || cur;

      // Detectar teleporte (wrap) — se a diferença > metade do grid, é wrap
      let dx = cur.x - prev.x;
      let dy = cur.y - prev.y;
      if (Math.abs(dx) > CFG.COLS/2) dx = dx > 0 ? dx-CFG.COLS : dx+CFG.COLS;
      if (Math.abs(dy) > CFG.ROWS/2) dy = dy > 0 ? dy-CFG.ROWS : dy+CFG.ROWS;

      _interp.positions.push({
        px: (prev.x + dx*t) * CFG.CELL,
        py: (prev.y + dy*t) * CFG.CELL,
      });
    }
  }
  ...
  function _loop() {
    ...
    animator.tick(now); shake.update();
    _computeInterp(now);
    ctx.save(); ctx.translate(shake.x,shake.y);
    _bg(); _grain(); _walls(now); _edgeDanger(now); _trail();
    particles.draw(ctx); _foods(now); _powerup(now);
    _snake(now); _deathCrumble(now);
  ...
  function _grain() {
    if(!_grainCanvas)return; if(Math.random()<.15)_makeGrain();
    const pat=ctx.createPattern(_grainCanvas,'repeat');
    ctx.globalAlpha=.6; ctx.fillStyle=pat; ctx.fillRect(0,0,canvas.width,canvas.height); ctx.globalAlpha=1;
  }

  function _walls(now) {
    if(!gameState?.walls?.length&&!_dynWallAnim.size)return;
    const c=CFG.CELL, head=gameState?.snake?.[0];

    // Limpeza de anim expiradas
    _dynWallAnim.forEach((anim,k)=>{
      if(anim.phase==='out'){
        anim.alpha=Math.max(0,1-(performance.now()-anim.born)/DYN_FADE);
        if(anim.alpha<=0)_dynWallAnim.delete(k);
      } else {
        anim.alpha=Math.min(1,(performance.now()-anim.born)/DYN_FADE);
      }
    });
    ...
    for(const w of (gameState?.walls||[])){
      ...
      if(isDyn){
        const anim=_dynWallAnim.get(k);
        if(anim) alpha=anim.alpha;
        else alpha=1; // fallback
      } else {
        if(head){const d=Math.abs(w.x-head.x)+Math.abs(w.y-head.y);if(d===1)alpha=.85;if(d===0)alpha=.6;}
      }
      ...
      if(isDyn){
        // Paredes dinâmicas: cor laranja + borda pulsante
        const pulse=0.7+Math.sin(now*.004+w.x+w.y)*.3;
        ctx.fillStyle='#ff6b35';
        ctx.shadowBlur=6*alpha*pulse; ctx.shadowColor='#ff6b35';
        ctx.fillRect(w.x*c+1,w.y*c+1,c-2,c-2);
        ctx.shadowBlur=0;
        // Timer — barra de depleção no topo do bloco
        if(w._born&&w._ttl){
          const elapsed=Date.now()-w._born;
          const pct=1-Math.min(elapsed/w._ttl,1);
        ...
        ctx.fillRect(w.x*c+1,w.y*c+1,c-2,c-2);
        ctx.fillStyle=CFG.BG; ctx.globalAlpha=alpha*.12;
  ...
  function _foods(now) {
    ...
    for(const f of gameState.foods){
      ...
      if(f.pts>=20){let angle=orbitAngles.get(f)||0;angle+=.06;orbitAngles.set(f,angle);for(let i=0;i<3;i++){const a=angle+(i/3)*MathUtil.tau;particles.spark(wx+Math.cos(a)*CFG.CELL*.7,wy+Math.sin(a)*CFG.CELL*.7,{color:f.color||CFG.FG,glow:5});}}
  ...
  function _foodBadge(f,pulse){
    if(f.pts<=2)return;
    ctx.font='bold 8px "Space Mono",monospace';ctx.fillStyle=f.color||CFG.FG;
    ctx.textAlign='left';ctx.textBaseline='bottom';
    ctx.scale(1/pulse,1/pulse);ctx.fillText(`+${f.pts}`,(f.r||4)+2,-(f.r||4)-2);
  }

  function _powerup(now){
    if(!gameState?.powerup)return;
    const pw=gameState.powerup;const def=POWERUP_TYPES.find(p=>p.kind===pw.kind);
  ...
  function _snake(now) {
    if(!gameState?.snake)return;
    const {dir,pwKind,pwActive}=gameState; const c=CFG.CELL;
    ...
    const positions=_interp.positions;
    ...
    for(let i=positions.length-1; i>=0; i--){
      ...
      ctx.save();
      // Interpola para o centro do bloco
      ctx.translate(pos.px+c/2, pos.py+c/2);

      if(pwActive&&!ghost){
        const def=POWERUP_TYPES.find(p=>p.kind===pwKind);
        if(def){ctx.shadowBlur=12;ctx.shadowColor=def.color;}
      }
      if(portal&&i<4){ctx.shadowBlur=10;ctx.shadowColor='#00d2ff';}
