...
const DOM = (() => {
  ...
  return {
    ...
    playerIdInput:  q('player-nick-input'),
...
// sons
    ...
    // Side rank (visível durante o jogo)
    sideRank:       q('side-rank'),
    rankListSide:   q('rank-list-side'),
    rankRefreshSide:q('rank-refresh-side'),
...
// ── Side Rank (visível durante o jogo) ───────────────────────────
async function _loadSideRank() {
  if (!DOM.rankListSide) return;
  DOM.rankListSide.innerHTML = '<div class="rank-loading">sincronizando...</div>';

  try {
    if (!window.FirebaseDB) throw new Error('offline');
    const rank = await window.FirebaseDB.loadRanking(5);
    console.info('[Rank] Dados recebidos:', rank);

    const myId = Store.getPlayerId();
    DOM.rankListSide.innerHTML = '';

    if (!rank || rank.length === 0) {
      DOM.rankListSide.innerHTML = '<div class="rank-loading">nenhum recorde ainda</div>';
      return;
    }

    rank.forEach((entry, i) => {
      const row = document.createElement('div');
      const isMe = entry.id === myId || entry.nick === myId;
      row.className = 'rank-row' + (isMe ? ' rank-me' : '');
      // nick vem do campo salvo; fallback para o id do documento
      const displayName = entry.nick || entry.id || '???';
      const score = entry.best_classic ?? entry.bestClassic ?? 0;
      row.innerHTML = `
        <span class="rank-pos">${i + 1}</span>
        <span class="rank-nick">${displayName}</span>
        <span class="rank-score">${score}</span>
      `;
      DOM.rankListSide.appendChild(row);
    });
  } catch(err) {
    console.warn('[Rank] Erro ao carregar:', err);
    DOM.rankListSide.innerHTML = '<div class="rank-loading">offline</div>';
  }
}

function _showSideRank() {
  if (DOM.sideRank) {
    DOM.sideRank.classList.remove('hidden');
    _loadSideRank();
  }
}

function _hideSideRank() {
  if (DOM.sideRank) DOM.sideRank.classList.add('hidden');
}
...
function _bindKeyboard() {
  ...
  document.addEventListener('keydown', e => {
    // Não intercepta nada enquanto o usuário está digitando em um input
...
function _initAuth() {
  ...
  const authenticate = async () => {
    ...
    if (window.FirebaseDB) {
      ...
      // Sempre sincroniza para garantir que o nick fica salvo no Firebase
      Store.syncToCloud();
    }
...
function _bindButtons() {
  ...
  if (DOM.rankRefreshSide) {
    DOM.rankRefreshSide.addEventListener('click', _loadSideRank);
  }
...
// (authenticate duplicado removido — a função correta está dentro de _initAuth)
// ── Resize handling ───────────────────────────────────────────────
